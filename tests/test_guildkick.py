...
async def test_caller_not_member(contract):
    ...
    with pytest.raises(Exception):
        await contract.submitGuildKick(member_address).invoke(caller_address=caller_address)
...
async def test_submitGuildKick(contract):
    ...
    return_value = await contract.submitGuildKick(member_address).invoke(caller_address=caller_address)
