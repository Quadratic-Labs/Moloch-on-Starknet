...
async def test_submit_onboard(contract):
    ...
    return_value = await contract.submitOnboard(address=member.address, accountKey=member.accountKey, shares=member.shares, loot=member.loot).invoke(caller_address=caller_address)
